...
public class Builder : Production
{
    const int ID = 112;
    ...
    public override void Init(int itemId)
    {
        if (!initOnce)
        {
            ...
            builderCoroutine = StartCoroutine(BuilderCoroutine());
            initOnce = true;
        }
    }
    ...
    private IEnumerator BuilderCoroutine()
    {
        while (true)
        {
            if (nextBelt == null)
    ...
    {
        StopCoroutine(builderCoroutine);
        SetOutputItemId(id);
